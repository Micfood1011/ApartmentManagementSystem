...
class PaymentsPage:
    ...
    def load_payments(self):
        ...
        cur = conn.cursor()
        cur.execute('''SELECT t.full_name, u.unit_number, p.amount, p.month_covered, p.payment_date
                      FROM payments p JOIN tenants t ON p.tenant_id = t.id 
                      LEFT JOIN units u ON t.unit_id = u.id ORDER BY p.payment_date DESC''')
        for row in cur.fetchall():
            self.tree_payments.insert("", "end", values=(row[0], row[1] or "—",
                                       f"₱{row[2]:,.2f}", row[3], row[4]))
        conn.close()
