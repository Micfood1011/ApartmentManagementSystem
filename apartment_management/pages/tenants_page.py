...
class TenantsPage:
    ...
    def load_tenants(self):
        ...
        cur = conn.cursor()
        cur.execute('''SELECT t.id, t.full_name, t.email, t.phone, u.unit_number,
                      t.move_in_date, t.monthly_rent FROM tenants t 
                      LEFT JOIN units u ON t.unit_id = u.id''')
        for row in cur.fetchall():
            unit = row[4] if row[4] else "—"
            rent = f"₱{row[6]:,.2f}" if row[6] else "—"
            self.tree_tenants.insert("", "end", values=(row[0], row[1], row[2] or "—",
                                     row[3] or "—", unit, row[5], rent))
        conn.close()
